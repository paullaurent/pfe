...
def main(robotIP, behaviorName):
  ...
  if (now.weekday()==0):
      ...
      phrase="Aujourd'hui c'est mardi"
  elif (now.weekday()==2):
      phrase="Aujourd'hui c'est mercredi"
  elif (now.weekday()==3):
      phrase="Aujourd'hui c'est jeudi"
  elif (now.weekday()==4):
      phrase="Aujourd'hui c'est vendredi"
  elif (now.weekday()==5):
      phrase="Aujourd'hui c'est samedi"
  else:
      phrase="Aujourd'hui c'est Dimanche"
...
if __name__ == "__main__":
    main("169.254.189.104","Stand/Gestures/This_2")
    if (len(sys.argv) < 3):
